...
func (w *workspacesResource) Operations() []registry.Operation {
	return []registry.Operation{
		...
		{
			Name:        "use",
			Description: "Set the default workspace stored in ~/.airbyte-cli/settings.json",
			Schema: registry.OperationSchema{
...
// useWorkspace updates Settings.Workspace in ~/.airbyte-cli/settings.json.
...
func useWorkspace(ctx context.Context, c *client.Client, params map[string]any) (any, error) {
	...
	return map[string]any{
		"status":    "saved",
		"workspace": canonical,
		"message":   fmt.Sprintf("default workspace set to %q in ~/.airbyte-cli/settings.json", canonical),
	}, nil
}
