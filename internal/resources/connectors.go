...
// user's configured default (from ~/.airbyte-cli/settings.json, exposed on the
...
var confirmDestructive = func(prompt string) error {
	if !isTerminal() {
		return client.NewValidationError(
			...
			"set \"allow_destructive\": true in ~/.airbyte-cli/settings.json (or AIRBYTE_ALLOW_DESTRUCTIVE=true) to allow non-interactive destructive operations",
		)
	}
