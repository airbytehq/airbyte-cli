...
How to cut a new release of `airbyte-agents`, what happens behind the scenes, and how to clean up if something goes wrong.
...
1. Mints a cross-repo token via the Octavia bot GitHub App (scoped to `airbyte-agents-cli` + `homebrew-tap`).
2. Runs `goreleaser release --clean`, which:
   - Builds cross-platform binaries (linux/darwin/windows × amd64/arm64).
   - Creates a **draft** GitHub release in `airbyte-agents-cli` with the archives and `checksums.txt` attached.
   - Commits an updated `Formula/airbyte-agents.rb` directly to `main` on `airbytehq/homebrew-tap`.
...
**GitHub UI:** open the draft from `https://github.com/airbytehq/airbyte-agents-cli/releases`, click the pencil/edit icon, scroll to the bottom of the editor, click **Publish release**.

**CLI:**

```
gh release edit vX.Y.Z --draft=false --repo airbytehq/airbyte-agents-cli
```

Once published, the tarball URLs in the brew formula resolve and `brew install airbytehq/tap/airbyte-agents` works. Before publishing, those URLs return 404 to anonymous requests (which is what `brew` makes).
...
**1. Delete the draft GitHub release** at `https://github.com/airbytehq/airbyte-agents-cli/releases`. From CLI:

```
gh release delete vX.Y.Z --repo airbytehq/airbyte-agents-cli --yes
```
