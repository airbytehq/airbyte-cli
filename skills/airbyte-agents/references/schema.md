...
- `airbyte agents schema` never makes API calls — safe to run without auth, against unfamiliar accounts, etc.
- Errors from `airbyte agents schema` (unknown resource or operation) are JSON on stderr with exit code 3.
- Operations that don't map to an OpenAPI route omit the `api` block. (`airbyte agents login` is also purely local but isn't a registered operation — it's a top-level command.)
