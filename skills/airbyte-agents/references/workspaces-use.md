...
Persist a default workspace name to `~/.airbyte-cli/settings.json`. After running this, any command that takes a `workspace` parameter and doesn't receive one will fall back to this value (in place of the literal `"default"`).
...
- **Right after `airbyte agents login`** — typically the second step in onboarding once you know which workspace you'll be working in.
...
```jsonc
{
  ...
  "message": "default workspace set to \"Production\" in ~/.airbyte-cli/settings.json"
}
```
